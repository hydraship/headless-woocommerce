...
/**
 * 
 * @TODO we will have to deprecate or remove this code since this is still mjs and we started moving the site info 
 * config to config.json which is generated by 000-settings-config.ts
 * 
 */
import fs from 'fs';
...
export default async function execute(params) {
  ...
  try {
    ...
    settingsWithDefaults.cartPageSlug = typesenseConfigs?.cartPageSlug?.value;
    settingsWithDefaults.wpPageHeadless = maybeConvertToBool(typesenseConfigs?.wpPageHeadless?.value);
    settingsWithDefaults.wpPostHeadless = maybeConvertToBool(typesenseConfigs?.wpPostHeadless?.value);
    settingsWithDefaults.showShareToPinterestButton = maybeConvertToBool(
      typesenseConfigs?.showShareToPinterestButton?.value
    );

    settingsWithDefaults.enableGeoRestrictions = maybeConvertToBool(
      typesenseConfigs?.enableGeoRestrictions?.value
    );
    ...
    settingsWithDefaults.isSubscriptionEnabled = maybeConvertToBool(
      typesenseConfigs?.isSubscriptionEnabled?.value
    );
    ...
    settingsWithDefaults.store.productPageSettings = parseJSON(
      typesenseConfigs?.productPageSettings?.value,
      {
        hotSale: 0,
      }
    );

    settingsWithDefaults.store.enableOverrideBestSeller = maybeConvertToBool(
      typesenseConfigs?.enableOverrideBestSeller?.value
    );
  } catch (error) {
    console.log(error);
  }

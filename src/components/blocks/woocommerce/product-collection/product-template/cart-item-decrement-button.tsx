...
import { CartItemSkeletonDecrementButton } from '@src/components/blocks/woocommerce/product-collection/product-template/cart-item-skeleton';
...
export const CartItemDecrementButton = ({ block }: CartItemDecrementButtonProps) => {
  ...
  if ('product-cart-item' === type) {
    ...
    if (loading) {
      return <CartItemSkeletonDecrementButton />;
    }
    return (
      <button
        className={cn(
